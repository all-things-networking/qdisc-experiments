...
def generate_graph(input_path, output_path, x_interval):
    ...
    for flowid, data_points in throughput_data.items():
        ...
        for pt in data_points:
            x_values.append(i * x_interval)
            y_values.append(pt / x_interval) # y axis is byte/sec
            i += 1
    ...
    with open("sender_parameter.json") as qdisc_conf_fp:
...
if __name__ == '__main__':
    ...
    x_interval = int(sys.argv[3]) # in sec, period between points
    generate_graph(input_path, output_path, x_interval)